...
if ( function_exists( 'bp_post_get_permalink' ) ) { // ugly ugly ugly hack to check for pre-1.2 versions of BP
	...
	<?php if ( invite_anyone_access_test() && ! bp_is_group_create() ) : ?>
		<p class="send-invitations-by-email-wrap clearfix"><?php printf( __( 'Want to invite someone to the group who is not yet a member of the site?', 'invite-anyone' ), ( Humanities_Commons::$society_id === 'hc' ) ? 'Humanities' : strtoupper( Humanities_Commons::$society_id ) ) ?> <a class="button send-invitations-by-email alignright" href="<?php echo bp_loggedin_user_domain() . BP_INVITE_ANYONE_SLUG . '/invite-new-members/group-invites/' . bp_get_group_id() ?>"><?php _e( 'Send invitations by email', 'invite-anyone' ) ?></a></p>
	<?php endif; ?>
